...
const getPreviewLabel = (keyCode: string): string | null => {
  if (keyCode.startsWith("Key") || keyCode.startsWith("Digit")) {
    return getKeyDisplayLabel(keyCode);
  }

  if (PREVIEWABLE_KEYS.has(keyCode)) {
    return getKeyDisplayLabel(keyCode);
  }
...
interface KeyboardContextType {
  ...
  notifyKeyInteraction: (keyCode: string) => void;
}
...
const KeyboardProvider = ({
  ...
  const playSynthKeyClick = useCallback(
    (keyCode: string, kind: "down" | "up") => {
      ...
      const keyIndex = keyCode.charCodeAt(keyCode.length - 1) || 0;
      const basePitch = 140 + (keyIndex % 12) * 8;
      const peakGain = kind === "down" ? 0.09 : 0.05;
      const duration = kind === "down" ? 0.09 : 0.06;

      oscillator.type = "square";
      oscillator.frequency.setValueAtTime(
        kind === "down" ? basePitch : basePitch + 34,
        now,
      );

      transient.type = "triangle";
      transient.frequency.setValueAtTime(basePitch * 2.1, now);
      transient.frequency.exponentialRampToValueAtTime(basePitch * 0.8, now + duration);
    ...
    [enableSound, resumeAudioContext],
  );
  ...
  const playSoundDown = useCallback(
    (keyCode: string) => {
      ...
      if (!audioBufferRef.current) {
        playSynthKeyClick(keyCode, "down");
        return;
      }
    ...
    [enableSound, playSynthKeyClick, resumeAudioContext],
  );
  ...
  return (
    <KeyboardContext.Provider
      value={{
        ...
        setReleased,
        lastPressedKey,
        notifyKeyInteraction: onKeyInteraction ?? (() => {}),
      }}
    >
      {children}
    </KeyboardContext.Provider>
...
const KeystrokePreview = () => {
  ...
  useEffect(() => {
    ...
    const previewLabel = getPreviewLabel(lastPressedKey);
    if (!previewLabel) {
    ...
    const showTimer = window.setTimeout(() => {
      setDisplayKey(previewLabel);
      setAnimationKey((prev) => prev + 1);
    }, 0);

    const hideTimer = window.setTimeout(() => {
      setDisplayKey((current) => (current === previewLabel ? null : current));
    }, 520);
      ...
      <AnimatePresence mode="popLayout">
        {displayKey && (
          <motion.div
            key={animationKey}
            layout
            initial={{ opacity: 0, scale: 0.42, y: 18, filter: "blur(16px)" }}
            animate={{
              opacity: 1,
            ...
            exit={{ opacity: 0, scale: 0.72, y: -18, filter: "blur(12px)" }}
            transition={{
              type: "spring",
              stiffness: 420,
              damping: 28,
            ...
            className="absolute flex min-w-40 items-center justify-center rounded-[2rem] border border-white/55 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(239,233,226,0.96))] px-10 py-7 font-mono text-6xl font-black uppercase tracking-[0.25em] text-neutral-800 shadow-[0_28px_90px_rgba(28,24,20,0.28)]"
          >
            <motion.span
              initial={{ opacity: 0, scale: 1.18, filter: "blur(10px)" }}
              animate={{ opacity: 0.6, scale: 1, filter: "blur(0px)" }}
              transition={{ duration: 0.08 }}
              className="text-center text-[clamp(2.75rem,7vw,6rem)]"
            >
              {displayKey}
            </motion.span>
          </motion.div>
...
export const Keyboard = ({
  ...
  return (
    <KeyboardProvider
      ...
      onKeyInteraction={onKeyInteraction}
    >
      <div
        ref={containerRef}
        className={cn(
          "mx-auto w-fit [zoom:0.9] sm:[zoom:1.2] md:[zoom:1.45] lg:[zoom:1.7] xl:[zoom:1.95]",
          className,
        )}
      >
