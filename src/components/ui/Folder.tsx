...
// ─── Folder Component (ported from react-bits) ───────────────────────────────
export default function Folder({
  ...
  const handleClick = () => {
    if (externalOpen === undefined) {
      ...
      if (!internalOpen) setPaperOffsets(Array.from({ length: maxItems }, () => ({ x: 0, y: 0 })));
    }
  };
  ...
  const getOpenTransform = (index: number) => {
    ...
    if (index === 2) return 'translate(-50%, -100%) rotate(0deg)';
    return '';
  };
  ...
  return (
    <div style={{ transform: `scale(${size})` }} className={className}>
      <div
        className={`group relative transition-all duration-200 ease-in cursor-pointer select-none ${!open ? 'hover:-translate-y-2' : ''}`}
        style={{ ...(open ? { transform: 'translateY(-8px)' } : {}) }}
        onClick={handleClick}
      >
        <div
        ...
        >
          ...
          {papers.map((item, i) => {
            const sizeMap = [
              open ? 'w-[70%] h-[80%]' : 'w-[70%] h-[80%]',
              open ? 'w-[80%] h-[80%]' : 'w-[80%] h-[70%]',
