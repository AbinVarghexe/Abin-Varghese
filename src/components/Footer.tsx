...
export default function Footer() {
  ...
  return (
    <footer className="bg-white dark:bg-black text-black dark:text-white pt-20 px-6 md:px-12 lg:px-20 overflow-hidden transition-colors duration-300">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-0 border-t border-black/20 dark:border-white/20 pt-12">
        {/* Left Section */}
        <div className="lg:col-span-4 flex flex-col space-y-8 lg:pr-12">
          <div className="flex items-center space-x-4">
            <div className="w-12 h-12 rounded-full border-2 border-black dark:border-white flex items-center justify-center font-bold text-2xl">
              A
            </div>
            <div>
              <p className="font-bold text-sm tracking-widest uppercase">UNIQUE DIGITAL EXPERIENCES</p>
              <p className="font-bold text-sm tracking-widest uppercase text-black/50 dark:text-white/50">// IMMERSIVE CRAFT</p>
            </div>
          </div>
          
          <p className="text-black/60 dark:text-white/60 text-sm max-w-sm leading-relaxed">
          ...
          <div className="space-y-4 pt-4">
            <p className="text-black/80 dark:text-white/80 text-sm">88 Digital Way, Kochi, India</p>
            <p className="text-black/80 dark:text-white/80 text-sm">hello@abinvarghese.art</p>
            <div className="flex space-x-4 pt-2">
              {socialLinks.map((link, idx) => (
                <Link key={idx} href={link.href} className="text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white transition-colors">
                  {link.icon}
                </Link>
              ))}
            </div>
        ...
        <div className="lg:col-span-4 flex flex-col justify-between lg:pl-12 pt-8 lg:pt-0">
          <div className="space-y-6">
            ...
            <p className="text-black/60 dark:text-white/60 text-lg max-w-sm">
              Share your ideas with us, and we'll begin turning your vision into reality today.
            </p>
          </div>

          <div className="flex flex-col md:flex-row justify-between items-end mt-20 text-xs tracking-widest text-black/40 dark:text-white/40 uppercase">
      ...
      <div className="mt-20 -mb-4 md:-mb-8 lg:-mb-12">
        <h1 className="text-[12vw] font-bold tracking-tighter leading-none whitespace-nowrap overflow-hidden select-none font-vina uppercase text-black dark:text-white transition-colors duration-300">
          Abin Varghese
        </h1>
      </div>
    </footer>
